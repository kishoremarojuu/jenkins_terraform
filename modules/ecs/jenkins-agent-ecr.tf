...
data "aws_iam_policy_document" "jenkins_agent_ecr_assume_policy" {
  statement {
    ...
    resources = ["arn:aws:iam::${var.ecr_account_id}:role/serverless-jenkins-ivd-agent"]
  }
}
...
locals {
  jenkins_java_agent_def = templatefile("${path.module}/templates/jenkins-java-agent.json.tpl", {
    ...
    container_image         = "871244369079.dkr.ecr.us-west-2.amazonaws.com/ivd-serverless-jenkins" #CHANGEMEE
    cpu                     = 512
    memory                  = 1024
  })
}
