...
// Jenkins Remote
data "aws_iam_policy_document" "jenkins_remote_execution_policy" {
  statement {
    effect    = "Allow"
    actions   = ["sts:AssumeRole"]
    resources = ["arn:aws:iam::${var.remote_agent_account_id}:role/jenkins-deploy-role"]
  }
}

resource "aws_iam_policy" "jenkins_remote_execution_policy" {
  name   = "${var.name_prefix}-jenkins-remote-execution-policy"
  policy = data.aws_iam_policy_document.jenkins_remote_execution_policy.json
}
...
// Jenkins Remote
resource "aws_iam_role_policy_attachment" "jenkins_controller_task2" {
  role       = aws_iam_role.jenkins_controller_task_role.name
  policy_arn = aws_iam_policy.jenkins_remote_execution_policy.arn
}
